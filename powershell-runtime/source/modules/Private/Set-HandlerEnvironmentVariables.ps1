...
function private:Set-HandlerEnvironmentVariables {
    ...
    Write-RuntimeLog 'Start: Set-HandlerEnvironmentVariables'
    Write-RuntimeLog "Received Headers: $($Headers)"

    # Set default TEMP environment variables for each invocation to ensure they're "clean" for each handler invocation
    Write-RuntimeLog 'Set default TEMP environment variables'
    $env:TEMP = '/tmp'
    $env:TMP = '/tmp'
    $env:TMPDIR = '/tmp'

    # Set AWS Lambda specific environment variables
    Write-RuntimeLog 'Set AWS Lambda specific environment variables'
